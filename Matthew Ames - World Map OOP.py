...
sink = Room("Sink.", "The sink is really weird. It has no knobs to turn the water one, and the faucet the water\n"
...
mwk = Room("Man with key.", "Upon further inspection, the figure is actually a man in a black trench coat \n"
           ...
           False, chestkey, "abshe", None, "rdum", "corner", False, george)
...
rdum = Room("Red Dumpster.", "When looking in the red dumpster, you see there is a flashlight.", False,
...
def use_menu():
    ...
    found = False
    try:
        if use_response == 'button' and button.used is True:
...
while True:
    if flashlight and batteries in inventory:
        Settings.flashlight_on = True
    if bum_joe.luck >= 2:
        Settings.real_penny = True
    if bum_joe.luck == 4:
        Settings.win_condition = True
    if current_node == pero and Settings.real_penny is True:
        print()
        pero.objects = penny

    if penny and nickel and quarter and dime in secinv:
        Settings.quest = True
        pls_print = True
        if pls_print is True:
            print("\n"
            ...
            pls_print = False
            secinv.remove(penny and nickel and quarter and dime)
    ...
    elif current_node == sink and Settings.toilet_used is True:
    ...
    if command in drop:
        response = input("Drop what?")
        if response in inventory:
            inventory.remove(response)
            print("You dropped %s" % response)
        else:
    ...
    if command in take:
        try:
            ...
            if response == "penny" and current_node == pero and bum_joe.luck >= 2:
